...
public class ANN{
	...
	public ANN(int nI, int nO, int nH, int nPH, double a,int actfunc,int Outputfunc)
	{
		numInputs = nI;
		numOutputs = nO;
		numHidden = nH;
		numNPerHidden = nPH;
		alpha = a;
        Activationfunctions.Add(actfunc);
        Activationfunctions.Add(Outputfunc);
	...
	public List<double> CalcOutput(List<double> inputValues)
	{
		...
		for(int i = 0; i < numHidden + 1; i++)
		{
				...
				for(int j = 0; j < layers[i].numNeurons; j++)
				{
					...
					if(i == numHidden)
						layers[i].neurons[j].output = ActivationFunction(N,1);
					else
						layers[i].neurons[j].output = ActivationFunction(N,0);
	...
	{
        double result=0.0f;
        switch (Activationfunctions[functocall])
        {
            case 0:
                result= Step(value);
                break;
            ...
            case 3:
                ...
                break;
            case 4:
                result = LeakyReLu(value);
                break;
            case 5:
                result = Sinusoid(value);
                break;
            case 6:
                result = ArcTan(value);
                break;
